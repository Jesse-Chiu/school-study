import{u as k}from"./index-Bvi4FBeA.js";import D from"./3-1-1-Ui4EIJu7.js";import M from"./3-1-2-Bb_5qz3K.js";import B from"./3-1-3-D9nlcxcw.js";import O from"./3-2-1-CnnGZ8n_.js";import C from"./3-2-2-C5aA1bly.js";import Q from"./3-2-3-CsQj7kDS.js";import H from"./3-2-4-Ci8avHwb.js";import L from"./4-1-1-rQ02kDiT.js";import G from"./4-1-2-01AJUFo3.js";import S from"./4-2-1-D0BfgThd.js";import T from"./4-2-2-DyGsNZY2.js";import F from"./4-2-3-DZDxlz3K.js";import I from"./4-3-1-DfIrV4QF.js";import R from"./4-3-2-CvsbiWQe.js";import W from"./4-4-1-Dhltvcho.js";import X from"./4-4-2-C4C7VEM4.js";import Y from"./4-4-3-Dz3cAUJJ.js";import Z from"./4-5-BAu_bDxt.js";const c=`
...
`,j={花的结构:c,开花和结果:c,传粉:c,消化系统:a,消化和吸收:a,小肠:a,心脏:d,心脏的结构:d,四腔:d,体循环:h,肺循环:h,血液循环:h,肺泡:y,气体交换:y,呼吸运动:y,肾单位:w,肾小球:w,尿液:w,种子的萌发:g,种子萌发:g,萌发:g,光合作用:z,呼吸作用:z};function E(t){for(const[e,o]of Object.entries(j))if(t.includes(e))return o}const m={};[D,M,B,O,C,Q,H,L,G,S,T,F,I,R,W,X,Y,Z].forEach(t=>{m[t.id]=t});function P(t){return t.replace(/<br\s*\/?>/gi,`
`).replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/\s+/g," ").trim()}function u(t,e){const o=P(t);return o.length<=e?o:o.substring(0,e-3)+"..."}function K(t,e=80){if(!t)return"";const o=t.match(/viewBox="([^"]*)"/),l=(o?o[1]:"0 0 200 150").split(/\s+/),n=parseFloat(l[2]||"200"),x=parseFloat(l[3]||"150")/n,f=Math.round(e*x);return`<img src="data:image/svg+xml,${encodeURIComponent(t).replace(/%2F/g,"/").replace(/%3A/g,":").replace(/%3D/g,"=").replace(/%22/g,"'").replace(/%20/g," ").replace(/%3C/g,"<").replace(/%3E/g,">").replace(/%23/g,"#")}" style="width:${e}px;height:${f}px;display:block;margin:4px 0;border-radius:6px;border:1px solid #e2e8f0" />`}function U(t){const e=m[t];if(!e)return"";const o=E(e.title);return o?K(o,140):""}function v(t,e=0){const o="  ".repeat(e);let i="";const l=t.title.replace(/^[一二三四五六七八九十]+、\s*/,""),n=u(t.content,120);if(i+=`${o}- **${l}**：${n}
`,t.keyTerms&&t.keyTerms.length>0){const r=t.keyTerms.slice(0,4);i+=`${o}  - 关键词：${r.join("、")}
`}if(t.children&&t.children.length>0)for(const r of t.children)i+=v(r,e+1);return i}function $(t,e=0){const o="  ".repeat(e);let i="";const l=t.title.replace(/^[一二三四五六七八九十]+、\s*/,""),n=u(t.content,300);if(i+=`${o}- **${l}**：${n}
`,t.keyTerms&&t.keyTerms.length>0&&(i+=`${o}  - 关键词：${t.keyTerms.join("、")}
`),t.children&&t.children.length>0)for(const r of t.children)i+=$(r,e+1);return i}function ht(t){const e=t&&t!=="all",o=e?k.filter(l=>l.id===t):k;if(o.length===0)return"# 未找到数据";let i="";e||(i+=`# 🌱 七年级下册生物 · 期末总复习

`);for(const l of o){const n=l.id==="unit3"?"🌿":"🫀";e?i+=`# ${n} ${l.title}

`:i+=`## ${n} ${l.title}

`;for(const r of l.chapters){e?i+=`## ${r.title}

`:i+=`### ${r.title}

`;for(const x of r.sections){const f=m[x.id];if(!f)continue;if(e?i+=`### ${f.title}

`:i+=`#### ${f.title}

`,e){const s=U(x.id);s&&(i+=`${s}

`)}if(f.learningGoals.length>0){const s=e?f.learningGoals:f.learningGoals.slice(0,2);i+=`**学习目标：**
`;for(const b of s)i+=`- ${b}
`;i+=`
`}const p=e?$:v;for(const s of f.knowledgePoints)i+=p(s,0);i+=`
`}}}return i}export{ht as generateMindMapMD};
