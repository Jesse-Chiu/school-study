...
import { BookOpen, Globe, Maximize2 } from 'lucide-react';
...
export default function HistoryMindMapPage() {
  ...
  useEffect(() => {
    ...
    async function initMarkmap() {
      ...
      // 动态导入历史生成器
      const { generateHistoryMindMapMD } = await import('../lib/history-mindmap-generator');

      if (cancelled) return;

      const md = generateHistoryMindMapMD(activeUnitId);
  ...
  return (
    <div className={fullscreen ? 'fixed inset-0 z-50 bg-white' : 'h-full'}>
          ...
          <div className="flex bg-slate-100 rounded-lg p-0.5">
            {tabs.map(t => {
              ...
              return (
                <button
                  key={t.id}
                  onClick={() => navigate(t.id === 'all' ? '/history/mindmap' : `/history/mindmap/${t.id}`)}
      ...
      `}>
        ...
        <span>👆 点击节点展开/折叠</span>
        {activeUnitId !== 'all' && (
          <span className="text-amber-600 font-medium">📚 单元详细脑图</span>
        )}
      </div>
